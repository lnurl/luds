...
Every value introduced in this part is encoded as [BIP-350](https://github.com/bitcoin/bips/blob/master/bip-0350.mediawiki) bech32m, under a distinct human-readable part per type:
...
* `ck1<...>` — a 65-byte recoverable ECDSA signature (`r ‖ s ‖ recovery-id`), produced by `WALLET` with the note's own `sk` over a fixed message (Offline verification). This is the actual bearer secret for a `cp1` note: showing or submitting it is what showing or submitting a plain `k1` did for a Part 1 secret, and it grants exactly the same thing to whoever holds it, full control of the note. There is only ever the one `ck1` per note, the same value whether it's submitted to redeem or shown to prove authenticity. 2-char HRP (`ck`) + 1-char separator + 104 data symbols (520 bits, divides evenly) + 6-char checksum = 113 characters total. This only ever appears in `k1`, in a callback or a note URL, never in `comment`. A merge combining `ck1`-bearing notes fits proportionally fewer per request than the ~68-byte-per-legacy-`k1` estimate in Redeeming a bearer note — budget roughly 12-17 such notes per merge under a 2000-character URL, rather than 20-30.
...
* `cx1<...>` — a 64-byte export of the safe-to-share derivation branch (Seed & derivation): the branch's public key `P` (32 bytes) concatenated with its chain code (32 bytes), nothing more. 2-char HRP (`cx`) + 1-char separator + 103 data symbols (512 bits) + 6-char checksum = 112 characters. Unlike `cp1`/`ck1`/`cs1`, this value never appears in a mint interaction — it isn't sent to `SERVICE` or embedded in a note URL, it's handed directly from `WALLET` to whichever companion device or tool should be able to derive `cp1_0, cp1_1, ...` for that branch.

### Wallet-side ownership proofs

A `SERVICE`/`WALLET` pair implementing this section replaces the hash-preimage commitment described in Protecting a freshly minted note and Redeeming a bearer note with a keypair: `WALLET` generates a private key `sk` and its public key `pk = sk·G`, and discloses only `pk`, encoded as `cp1<pk>`, wherever Part 1 would have disclosed `hex(sha256(secret))`.

* **Minting.** `comment = cp1<pk>` in place of `hex(sha256(secret))`; `SERVICE` credits the note as `k1 = cp1<pk>` directly. No separate hashing step is needed, the public key is already a non-revealing commitment to `sk`, exactly as `sha256(secret)` was a commitment to `secret`.
* **Rotate/split/merge output.** `p1` (and `p2`) carry `cp1<pk_new>` directly, in place of a hash of a not-yet-disclosed preimage.
* **Redemption.** Instead of revealing `sk`, `WALLET` submits `k1 = ck1<sig>` (Offline verification): a recoverable ECDSA signature over a fixed message, the same value every time for a given key. Knowledge of a valid `ck1` is itself the authorization, exactly as knowledge of `secret` was in Part 1 — it doesn't need to cover the specific `pr`/`amount`/`p1`/`p2` of any one request, any more than Part 1's `secret` did. `SERVICE` recovers the signer's public key with `ecrecover(digest, sig)`, takes its x-coordinate, and looks that up against its own outstanding-notes table, the same lookup it already does for a plain hex `k1`. No pubkey needs to be resent alongside the signature, recovery finds it. A merge combining several `cp1` notes needs no positional pairing either: every `k1 = ck1<sig>` entry recovers to its own note independently, in any order.
...
A verifiable note is the pair `(ck1<...>, cs1<...>)` alongside its `amount_msat`. `ck1` is exactly the same signature used to redeem the note (Wallet-side ownership proofs): showing one is not a lesser, display-only gesture, it hands over full spending capability, exactly as exposing a Part 1 `secret` would. `cs1` is `SERVICE`'s attestation, produced the same way [LUD-13](13.md) signs its auth seed phrase: a recoverable ECDSA signature with deterministic ([RFC6979](https://www.rfc-editor.org/rfc/rfc6979)) nonces, over

```
message = "LNURLcash:" || amount_msat (decimal ASCII) || ":" || hex(pk)
digest  = sha256(sha256("Lightning Signed Message:" || message))
```

`hex(pk)` is the raw 32-byte key that `p1` (or `p2`) carries as `cp1<pk>` on the redeem callback; `SERVICE` decodes it and signs over the raw key directly, no hashing needed.
...
`WALLET` computes this once per key and reuses the exact same value everywhere `ck1` is needed, redemption included (Wallet-side ownership proofs): there is no separate display-only variant, only the one signature that already does both jobs. No domain tag is needed to keep it apart from `cs1`, `cs1` is always checked against `mintPubkey`, a key `ck1` could never recover to, so the two are never confusable regardless of message content.

To verify offline, a `WALLET` (or any recipient) first recovers `pk` from `ck1` — computable with no external input at all, since the message it signs never changes. It then recomputes `cs1`'s message using that recovered `pk` and the URL's declared `amount`, and recovers a second key from `cs1`, comparing it to the `mintPubkey` it has on record for that mint. A match on both proves the note was issued by that mint for that amount. It proves nothing about *who* is showing it, and it can't: showing `ck1` at all already means handing over the note, there is no lesser act of merely proving authenticity without also transferring it.
...
**What this does and does not prove.** Showing a verifiable note transfers the note, exactly as showing a Part 1 bearer note would; `SERVICE` never cares who submits a `ck1`, only that it recovers to a note still outstanding. What `cs1` adds is a check the *recipient* can make before accepting that transfer: that the note really was issued by this mint, for this amount, rather than fabricated. It proves issuance, not that the note is still outstanding, a spent note keeps its valid `cs1` forever, and no revocation is visible offline. So accepting one is still a trust decision, just a narrower one than in Part 1: not "is this even real," which `cs1` now answers, but "has the person handing it to me already spent it or shown it to someone else," exactly as checking a banknote for counterfeits does not prove it was not already promised to someone else. Once accepted, the new holder should rotate immediately, for the same reason Offline circulation already recommends it for Part 1 notes; the only definitive settlement is an online rotate.

### Seed & derivation

Every private key `WALLET` uses under Wallet-side ownership proofs, a mint's `sk`, or a rotate/split/merge's replacement key, is, as specified so far, plain randomness. `SERVICE` never sees or needs it, so that's fine for `SERVICE`, but it's a real gap for `WALLET`: an ordinary HD wallet backup is just a seed phrase, and a seed phrase backs up nothing that was never derived from it.

`WALLET` SHOULD instead derive these keys deterministically. The branch root for a given `SERVICE` is derived hardened, reusing the exact domain-hashing steps [LUD-05](05.md) already defines for its `linkingKey`, under a separate purpose so the two never share key material:
