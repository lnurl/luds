...
A GET on the withdraw LNURL carrying a specific note's `k1` behaves exactly as the informational query described in Redeeming a bearer note, `{"tag": "withdrawRequest", "callback": string, ...}` plus `mintPubkey` for Offline verification.
...
`SERVICE` MAY additionally accept `?h=<hex sha256 of k1>` in place of `?k1=` on the informational GET, **and only there, never at the `callback`**. This is the same letter the `callback` uses for `h`/`h2`, `SERVICE` already keys every note it registered by hash internally, every mint and every rotate/split/merge output are stored that way already, this is just a second way in for a lookup it can already do.
...
An unknown response to a hash lookup therefore does not distinguish a spent note from one that was never registered. Because hash lookup is optional, it may also mean that `SERVICE` does not support this lookup form. `WALLET` MUST NOT treat this response alone as proof that a note was spent, never existed, or that a requested mutation completed. It MUST NOT discard held or replacement note secrets on the strength of this response alone. It SHOULD describe the result as an inconclusive status check, retaining the note's last known value as such rather than presenting it as a newly confirmed balance.

For a melt initiated by `WALLET`, a matching, settled payment verification response, where supplied by `SERVICE` (see [LUD-21](21.md)), can establish payment completion independently of this lookup. To distinguish a spent note from an unknown one when no such proof is available, `WALLET` MAY offer an explicit informational lookup using `k1`. It SHOULD explain that this discloses the bearer secret and, if the note remains outstanding, rotate it into a fresh wallet-generated note. An unknown hash response alone MUST NOT trigger an automatic secret-disclosing retry. A timeout, pending response, or inconclusive check is not evidence of a completed payment and MUST NOT cause the note's secret to be discarded.
...
For a rotate, split or merge, the `WALLET` generates the new note's secret: it generates a fresh 32-byte preimage, the same size a Lightning payment preimage already is at mint time (ideally seed-derived rather than merely random, see Seed-recoverable note secrets), and discloses only its hash, as `h` (and, for a split's second output, `h2`). `SERVICE` registers the new note keyed by that hash directly and produces `sig`/`sig2` over that hash (see Offline verification). At no point does `SERVICE` see, generate, or need the raw preimage; it MUST NOT persist anything beyond the hash it was given. `h` MUST be present on every request that lacks `pr`, and `h2` MUST additionally be present whenever `amount` is (a split); `SERVICE` MUST reject a request missing a required hash with `{"status": "ERROR", "reason": "missing h"}` (or `"missing h2"`).

The callback response is the [LUD-03](03.md) success response: `{"status": "OK"}`, plus `sig` (and `sig2` for a split), `SERVICE`'s attestation over `h` (and `h2`); see below. As with any `withdrawRequest`, an invalid, already-spent, or unknown `k1` MUST yield `{"status": "ERROR", "reason": string}`, unless it's a retry of a mutation `SERVICE` already completed (Retrying a mutation); if *any* `k1` in a multi-`k1` request is invalid, the whole request MUST fail and no note may be burned or minted.
...
**A retried rotate, split or merge MUST be answered as a replay, not an error.** A GET gets retried by transports that never ask `WALLET`'s permission first: an HTTP client's own timeout-retry logic, a proxy in between, a flaky connection resending the request it never saw a reply for. A rotate/split/merge is exactly the kind of GET that mutates state once and only ever wants to say so once, so `SERVICE` MUST recognize when it's being asked about a burn it already performed rather than treating the now-burned `k1` as an ordinary already-spent one. Concretely: if the `k1`(s), `h`, `h2` and `amount` of an incoming request exactly match those of a burn `SERVICE` already completed, `SERVICE` MUST return the same `{"status": "OK", "sig": ..., "sig2": ...}` it returned the first time, not `{"status": "ERROR", "reason": "already spent"}`. A request that repeats the `k1`(s) but disagrees with the recorded burn on `h`, `h2`, or `amount` is a genuine conflict, not a replay, and still gets the plain already-spent error.

This isn't a new storage burden: `SERVICE` MUST already remember a `k1` as burned forever, to refuse ever reissuing it (above), so keeping the `h`/`h2`/`sig`/`sig2` that burn produced alongside it, for exactly as long, is the same piece of state, not an additional one. Without this rule, a rotate/split/merge whose response is lost in transit is indistinguishable, to both `WALLET` and `SERVICE`, from an attempt to reuse an already-spent note: `WALLET` gets an error for a note `SERVICE` itself just minted, and the only copy of that note's value looks lost to `WALLET` even though `SERVICE` is still holding it outstanding under `h`.
