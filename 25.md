...
A minted note's `k1` is nothing but the payment preimage `P`. That is normally fine, `P` is secret until the payer reveals it, but `payRequest` ([LUD-06](06.md)) commonly pairs with [LUD-21](21.md) `verify` for the unrelated purpose of letting a payer prove *that invoice* got paid, and `verify` responses are handed out, logged, and cached far more casually than a bearer note ever should be. If `SERVICE` also serves `verify` on the mint `payRequest`, it discloses `preimage: P` to anyone holding the `verify` URL the moment the invoice settles, whether or not they are the payer, which is enough on its own to redeem or rotate the note out from under whoever actually paid for it. Two unrelated things, "prove this invoice was paid" and "hold the bearer note it minted", end up secured by the same one secret.

`WALLET` closes this by making sure `P` alone is never sufficient to claim the note. Before paying, it generates a second, independent random 32-byte `secret` (the same size `P` and every other `WALLET`-generated preimage in this document already are) and encrypts it to `SERVICE`'s node pubkey, the same identity it already recovers from the invoice's own recoverable signature in order to pay it (see Offline verification), so no extra round trip or published key is needed just for this. A `WALLET` that implements `LNURLcash` MUST do this on every mint payment; it costs nothing when `SERVICE` doesn't support it (see fallback below), and the alternative is a `WALLET` that has to remember, per-`SERVICE`, whether it's safe to skip.

Encryption reuses the construction every Lightning node already ships for its own transport, [BOLT#8](https://github.com/lightning/bolts/blob/master/08-transport.md)'s Noise handshake, rather than an app-layer scheme foreign to Lightning: `WALLET` generates an ephemeral secp256k1 keypair, computes `ECDH(ephemeral_privkey, SERVICE_node_pubkey)`, derives a 32-byte key from that shared secret with `HKDF-SHA256` exactly as [BOLT#8](https://github.com/lightning/bolts/blob/master/08-transport.md) derives its own handshake keys, and encrypts `secret` under that key with `ChaCha20-Poly1305` (random 12-byte nonce, the AEAD tag doubles as integrity, no separate MAC). The value attached as `comment` ([LUD-12](12.md)) is `hex(ephemeral_pubkey[33B] || nonce[12B] || ciphertext_with_tag)`; a mint `payLink` intending to support this SHOULD advertise a `commentAllowed` large enough to carry it (a bit over 100 bytes, comfortably inside the budget [LUD-12](12.md) already allows).

* If `SERVICE` receives no `comment` on a mint payment, or fails to decrypt one, it MUST fall back to crediting the note as `k1=P` exactly as described above, and MUST NOT offer `verify` for that invoice, since `P` is at that point the entire bearer secret and handing it out defeats the point.
* If `SERVICE` receives and decrypts a `comment` into `secret`, it MUST credit the note as `k1=<hex(secret)>:<hex(P)>` instead of `k1=P` alone, and MUST require both parts, matched exactly, to redeem it. `SERVICE` MAY now also serve `verify` for that invoice: a `verify` response only ever discloses `P`, the half of the pair `SERVICE` never needed to keep confidential, since `secret` never left `WALLET` in the clear and `SERVICE` only ever saw it inside a ciphertext addressed to a key nobody else holds. A `verify` request for a payment hash `SERVICE` has no decrypted `secret` for (including one it never offered `verify` on in the first place, per the fallback above) MUST fail exactly as an unknown one would, `{"status": "ERROR", "reason": "not found"}`, indistinguishable from `SERVICE` never having implemented `verify` for that invoice at all.

Nothing here needs `WALLET` to implement `LNURLcash` at all, only plain [LUD-12](12.md) comments: a holder can generate `secret`, encrypt it out of band, and hand the ciphertext to anyone, even someone with an entirely ordinary Lightning wallet, to paste into `comment` and pay, exactly like handing them an invoice to settle on your behalf. That payer's wallet never sees `secret` unencrypted, so even a wallet that goes on to call `verify` and recovers `P` learns nothing that redeems the note by itself, and the note remains claimable only by whoever holds `secret`. The composite `k1` is still just an opaque string, so this needs no new encoding step either, exactly as a bare `P` didn't.
...
For a rotate, split or merge, the `WALLET` generates the new note's secret: it generates a fresh random 32-byte preimage, the same size a Lightning payment preimage already is at mint time, and discloses only its hash, as `h` (and, for a split's second output, `h2`). `SERVICE` registers the new note keyed by that hash directly, if it signs notes, produces `sig`/`sig2` over that hash (see Offline verification). At no point does `SERVICE` see, generate, or need the raw preimage; it MUST NOT persist anything beyond the hash it was given. `h` MUST be present on every request that lacks `pr`, and `h2` MUST additionally be present whenever `amount` is (a split); `SERVICE` MUST reject a request missing a required hash with `{"status": "ERROR", "reason": "missing h"}` (or `"missing h2"`).
...
`SERVICE` MAY prove a melt happened by attaching a [LUD-21](21.md) `verify` to it, extending a melt's `withdrawSuccessResponse` the same way LUD-21 extends a `payRequest` callback:

```diff
 {
   "status": "OK",
+  "pr": string,      // the invoice this melt is paying, echoed back
+  "verify": string   // LUD-21 verify URL for that pr
 }
```

Once the outgoing payment settles, GET on `verify` returns the [LUD-21](21.md) response, `{"status": "OK", "settled": true, "preimage": ..., "pr": ...}`. Because a BOLT-11 `pr` commits to `payment_hash = sha256(preimage)`, anyone holding both `pr` and the returned `preimage`, not just `SERVICE`, can independently confirm the melt: decode `payment_hash` from `pr`, hash the `preimage`, and compare. This is proof a real Lightning payment settled *that* invoice, not merely `SERVICE`'s word for it, the same kind of gap Offline verification closes for minting, but for the melt side: a `WALLET` that melted a note can show a third party `pr`, the returned `preimage`, and the `k1` it burned, as a self-contained receipt that `SERVICE` doesn't need to vouch for.
