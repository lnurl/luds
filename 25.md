...
`WALLET` SHOULD derive keys deterministically. The branch root for a given `SERVICE` is derived hardened, reusing the exact domain-hashing steps [LUD-05](05.md) already defines for its `linkingKey`, under `m/139'`'s own `1'` sub-purpose so this section's branches never share a node with anything else a `WALLET` derives off `m/139'` for the same `SERVICE` - a deterministic Part 1 `secret` scheme, for instance, is not defined by this document but is a natural extension of the same seed, and would otherwise land on the exact same node this section derives:

```
addressRoot      = derive(masterKey, m/139'/1')                     // this section's own sub-purpose
cashHashingKey   = derive(addressRoot, 0)                           // LUD-05's step 1, own purpose
...
p, chaincode     = derive(addressRoot, d1/d2/d3/d4)                 // this SERVICE's branch root, hardened
P                = p·G
```
...
sequenceDiagram
    ...
    Alice->>SERVICE: GET callback: k1=ck1<sig>, p1=cp1<pk_i> (no pr)
