...
`hex(pk)` is the raw 32-byte key that `p1` (or `p2`) carries as `cp1<pk>` on the redeem callback; `SERVICE` decodes it and signs over the raw key directly, no hashing needed. `amount_msat (decimal ASCII)` is its minimal base-10 form: no leading zeros, no sign, digits only (`0` itself is `"0"`). A verifier reconstructing `message` from a non-canonical `amount` just recovers a different, non-matching key.
