...
+  "sig": string        // cp1 notes only: cs1<...>, mint attestation encoding this note's current amount
 }
```

A rotate, split or merge additionally delivers a fresh certificate in its `withdrawSuccessResponse`, since the note it produces doesn't exist to query until the mutation completes. None of these need a further rotate afterward just to pin down the resulting amount: `WALLET` already knows it, it's whatever it asked for (a split's `amount`, or a merge's deterministic post-fee sum, Mint fee's for split and combine), and the certificate confirming it arrives in that same response.
