...
The response is the same shape [LUD-03](03.md) already defines for the informational GET with `k1` omitted: the convenience that field normally serves, letting a `WALLET` copy the queried value straight into a `callback` request, does not apply here. A `WALLET` querying by hash already holds the raw value.

`SERVICE` MUST retain each registered note's hash and its spent state after burning it. An informational lookup with `k1`, or with `h` where supported, MUST distinguish a known spent note from an unknown note: return `{"status": "ERROR", "reason": "Note already spent."}` for the former and `{"status": "ERROR", "reason": "Unknown note."}` for the latter. A note with an unresolved melt MUST return `{"status": "ERROR", "reason": "pending"}`; a melt confirmed unpaid restores its outstanding value. Looking up any of these states MUST NOT burn, reissue or otherwise change the note.

A `WALLET` can therefore reconcile a spent note by hash without disclosing its bearer secret. Hash lookup protects the spending secret; it does not hide the note's existence or spent state from someone who already knows its hash. An unknown response alone is not proof of spending or of a completed mutation: the identifier may never have been registered, or an older `SERVICE` may not support this lookup form or distinguish spent hashes. `WALLET` MUST NOT discard held or replacement note secrets on the strength of an unknown response alone, and MUST NOT automatically retry that response by disclosing `k1`. For a melt it initiated, `WALLET` SHOULD use matching payment settlement verification where available; a pending response or transport failure is not confirmation that the melt settled.
