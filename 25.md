...
* `cx1<...>`: a 64-byte export of the watch-only derivation branch (Seed & derivation): the branch's public key `P` (32 bytes) concatenated with its chain code (32 bytes), nothing more. 2-char HRP (`cx`) + 1-char separator + 103 data symbols (512 bits) + 6-char checksum = 112 characters. It grants no spending authority, but is not private: it makes every note key on the branch enumerable and linkable. A `cx1` may be provisioned at a `SERVICE` and published for Internal transfer; it never identifies or authorizes spending of one individual note.
...
`SERVICE` itself MAY be handed a `cx1`, not just a passive watcher: once one is provisioned against a chosen `username`, `SERVICE` can publish an ordinary Lightning Address ([LUD-16](16.md)) at `https://<domain>/.well-known/lnurlp/<username>`, on the same domain as its mint, and mint automatically for every payment it receives there. Since `pk_i` is computable from `cx1` alone, `SERVICE` derives the next unused one itself and credits the note under `k1 = cp1<pk_i>`, exactly as if `WALLET` had supplied `comment = cp1<pk_i>` in person; no comment, and no `WALLET` involvement at receive time, is needed at all. `WALLET` discovers what arrived the same way it already recovers any note, by re-deriving `pk_0, pk_1, ...` and checking each (above). How a `SERVICE` allocates usernames or authorizes provisioning, replacement and removal of their branches is outside this document: control of a `cx1` proves neither entitlement to a human-readable username nor ownership of an optional Nostr identity. A management protocol MUST authenticate the current holder before replacement/removal, prove possession of any replacement branch, bind the complete resulting record and `SERVICE` origin into what is signed, and prevent replay. In particular, an update authorization signed by only the current branch over an action and username does not bind the replacement record, so it can be replayed with a substituted branch.
...
> [!WARNING]
> Publishing `cx1` makes every public key on that branch derivable by everyone, not only the intended payer. Anyone can correlate the branch's notes and probe `SERVICE` for their value and state. A `WALLET` SHOULD therefore use a distinct branch for each public receiving identity or account, explain this linkability before enabling the feature, and never publish the branch holding unrelated private notes.
