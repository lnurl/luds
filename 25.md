...
LN address registration proof (Seed & derivation): `WALLET` registers `username = "alice"` against this branch's `cx1`, signing with `sk_0` from the table above over `sha256("LNURLcash:register:alice")` (a later unregister uses the same `sk_0` over `sha256("LNURLcash:unregister:alice")` instead - a different digest, so one can never be replayed as the other):

```
username:                alice
register message:        LNURLcash:register:alice
register digest (sha256): 07511b102748f3a3614676386cf9ece7a5fcf11576df21d154bafbaeed77e7ae
...
unregister digest (sha256): a420c6e3107fb17638baf3b73d4f74f04312f331a4726f6e563a7d6aa3a5bba0
...
A verifier checks `Verify(pk_0, sha256("LNURLcash:register:alice"), sig)` (or the `unregister` message, for that action) against `pk_0` derived from the `cx1` on file - the same `Verify` Wallet-side ownership proofs and the paragraph below already use, just over a different, action- and username-bound digest.
