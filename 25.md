...
`ck1` is produced by `WALLET` differently: `sig = Sign(sk, sha256("LNURLcash"))`, a plain [BIP-340](https://github.com/bitcoin/bips/blob/master/bip-0340.mediawiki) Schnorr signature with the note's own `sk` over the fixed 32-byte digest `sha256("LNURLcash")`, independent of `pk` or `amount`. The message is hashed rather than signed as the raw 9-byte ASCII string because [BIP-340](https://github.com/bitcoin/bips/blob/master/bip-0340.mediawiki)'s own reference implementation, and most conforming Schnorr signers (`libsecp256k1`'s `schnorrsig` module included), only accept a 32-byte message; hashing first keeps `ck1` signable by an off-the-shelf signer instead of requiring a custom one just for this document. `WALLET` computes this once per key and reuses the exact same `(pk, sig)` pair everywhere `ck1` is needed, redemption included (Wallet-side ownership proofs), encoded together as `ck1<pk><sig>` (Encoding).

To verify offline, a `WALLET` (or any recipient) first reads `pk` directly out of `ck1` and checks `Verify(pk, sha256("LNURLcash"), sig)` (Wallet-side ownership proofs). It then decodes `amount_msat` straight from `cs1`'s own human-readable part (Encoding), recomputes `cs1`'s message using that amount and `pk`, and recovers a key from `cs1` (still a recoverable ECDSA signature, unchanged from above), comparing it to the `mintPubkey` it has on record for that mint. A match on both proves the note was issued by that mint for that amount.
...
A verifier checks `Verify(pk_0, sha256("LNURLcash:register:cash.example.com:alice"), sig)` (or the `unregister` message, for that action) against `pk_0` derived from the `cx1` on file - the same `Verify` Wallet-side ownership proofs and the paragraph below already use, just over a different, action-, domain- and username-bound digest. `SERVICE` supplies its own `domain` here, never a value `WALLET` passes it - the same string it already used as `hmacSha256`'s message in this vector's own domain-hashing step, above.

### Test vector 3: Wallet-side ownership proof (`ck1`)

Using `sk_0`/`pk_0` from test vector 1. `WALLET` signs `sha256("LNURLcash")`, the fixed message hashed to a 32-byte digest (Offline verification), with a plain BIP-340 Schnorr signature, **using an all-zero 32-byte `aux_rand`** - not for the usual side-channel-hardening reason BIP-340 documents `aux_rand` for, but so the exact same `(pk, sig)` pair comes out every time this key signs, deterministically, the same way `sk_i` itself is already deterministic; a `WALLET` re-deriving this during a recovery scan must land on the byte-identical `ck1` it minted the note under, not merely a validly-different one.
...
message:                     "LNURLcash"
digest (sha256(message)):    49a9bb7cae28a0c1f77bc7fac7693456b1cc149f83c413acfd938dc95ea21cf5
...
A verifier checks `Verify(pk, sha256("LNURLcash"), sig)` directly against the `pk` embedded in `ck1` (Wallet-side ownership proofs) - no recovery, no network round trip.
