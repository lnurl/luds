...
This document is in two parts. Part 1 is the complete, minimal scheme: plain secrets, plain hashes, nothing else, and everything a `SERVICE`/`WALLET` needs to interoperate. Part 2 is optional and purely additive: offline verification and a wallet-side ownership-proof scheme, both built on recoverable ECDSA signatures. It's not required for interoperability, a `SERVICE` or `WALLET` implementing only Part 1 interoperates fully with one that also implements Part 2, and nothing in Part 2 changes Part 1's wire format for a party that doesn't use it, but it's highly recommended regardless: without it, accepting a note offline is a leap of faith with no way to check it.
...
```diff
 {
   ...
   "callback": string,
   ...
   "metadata": "[[\"text/plain\",\"Mint fees: 1000,2000\"]]",
+  "withdrawLink": string,
+  "commentAllowed": 64
 }
```
...
If the `payLink` is published as a Lightning Address ([LUD-16](16.md)) at `https://<domain>/.well-known/lnurlp/<identifier>`, a `SERVICE` MAY instead publish its withdraw LNURL at the matching `https://<domain>/.well-known/lnurlw/<identifier>` (`_` for the default identifier), the withdraw-side mirror of that same [LUD-16](16.md) convention. A `SERVICE` additionally implementing Part 2 MAY let a `WALLET` register a Lightning Address this way without needing to pay itself for every note; see Seed & derivation.
...
A `WALLET` additionally implementing Part 2 can obtain a mint-signed certificate for a note; see Offline verification.
...
`WALLET` computes this once per key and reuses the exact same value everywhere `ck1` is needed, redemption included (Wallet-side ownership proofs). No domain tag is needed to keep it apart from `cs1`: `cs1` is always checked against `mintPubkey`, a key `ck1` could never recover to, so the two are never confusable regardless of message content.
