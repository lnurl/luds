...
Once the payment settles against a valid `comment = hex(h)`, `SERVICE` MUST credit the note as `k1=<hex(secret)>`. Redeeming it later needs nothing but `secret`, checked against `sha256(secret) == h`, exactly like redeeming any other `WALLET`-generated note (Redeeming a bearer note).
...
`SERVICE` SHOULD implement plain [LUD-21](21.md) `verify` on the mint `payRequest`. It is useful for `WALLET`, it can easily check when a mint or melt has settled.
