...
When `pr` was provided (melt), the response is the plain [LUD-03](03.md) `{"status": "OK"}`, no new note is minted. Otherwise `k1` MUST be present: for rotate it is the single replacement note; for split it carries the requested `amount`, and `change` carries what is left over, minus `base_fee_msat` if `SERVICE` charges mint fees. As with any `withdrawRequest`, an invalid, already-spent, or unknown `k1` MUST yield `{"status": "ERROR", "reason": string}`; if *any* `k1` in a multi-`k1` request is invalid, the whole request MUST fail and no note may be burned or minted.
