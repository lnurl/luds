...
Prefixed with the `lnurlw://` scheme ([LUD-17](17.md)) or bech32-encoded as an ordinary LNURL, `<withdraw LNURL>?k1=<spend>` *is* the note. It MAY carry `&amount=<msat>`, so a recipient can display its value without contacting `SERVICE`, and `&c=<cs1>` (Offline verification). `SERVICE` MUST ignore both at the informational endpoint; the authoritative value is always `maxWithdrawable` there.
...
+  "c": string           // cs1<...> certificate for the queried note
 }
```
...
`SERVICE` hands out certificates as `c` on the informational GET (for the queried note) and on every rotate, split or merge:

```diff
 {
   "status": "OK",
+  "c": string,   // cs1<...> certificate for p1
+  "c2": string   // cs1<...> certificate for p2, present if p2 was supplied
 }
```

A certified note travels as `lnurlw://mint.example/w?k1=<spend>&c=<cs1>`. To verify it offline, a recipient:
...
sequenceDiagram
    ...
    Alice->>Bob: hands over lnurlw://mint.example/w?k1=A&c=cs1
