...
The `status` property should be of type String and be equal to `settled`. In order for the receiving server to ensure that the payment was indeed settled the `SERVICE` must pass the `preimage` property on the payload object. To verify it the server could simply hash the `preimage` and compare it with the `paymentHash`. This payment hash must be the same as the payment hash that is encoded in the initial `invoice` provided to `WALLET`.
