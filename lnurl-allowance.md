...
- What is the message flow for `extending the current allowance amount/time`?
...
5. `LN WALLET` initiates a WebSocket connection to the `socket` URI provided in previous LNRUL response, and if connection is successful `LN WALLET` sends first identifying JSON strigified payload message:
	...
	```
	wss://server.btc/websocket-server
...
8. `LN WALLET` must now be responsible for:
  - 1) allowance time: current time < (`SERVICE` timestamp + `LN WALLET` `maxAllowanceTime`)
  - 2) allowance amount: current amount spent < `LN WALLET`'s `maxAllowanceAmount`
