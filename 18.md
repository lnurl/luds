...
If `SERVICE` wants to get one or more types of payer identities from `WALLET` then it MUST alter its JSON response to the first callback to include a `payerData` field, as follows:
...
If `WALLET` sends more `payerdata` fields than requested by `SERVICE`, `SERVICE` should still append everything as received.
