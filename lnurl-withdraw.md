...
5. Once accepted by the user, `LN WALLET` sends a GET to `LN SERVICE` in the form of
	...
	```
6. `LN SERVICE` verifies `k1` is valid, sends a `{"status": "OK"}` or `{"status": "ERROR", "reason": "error details..."}` JSON response and then attempts to pay the invoices asynchronously.
